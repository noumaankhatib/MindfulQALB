...
import { MessageCircle, X, RotateCcw, ChevronRight, Clock, CheckCircle } from 'lucide-react';
import { chatbotFlow, ChatStep, SessionRecommendation, getStepNumber, getTotalSteps } from '../data/chatbotFlow';
...
const Chatbot = () => {
  ...
  const handleOptionSelect = (optionLabel: string, nextStepId?: string) => {
    // Add user's response
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-50 w-[360px] max-w-[calc(100vw-48px)] h-[520px] max-h-[calc(100vh-100px)] bg-white rounded-2xl shadow-2xl border border-lavender-100 flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="bg-gradient-to-r from-lavender-500 to-lavender-600 text-white p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-sm">Session Guide</h3>
                  <p className="text-xs text-white/80">Find your perfect session</p>
                </div>
              </div>
              ...
              {currentStep?.type === 'question' && currentStep.options && (
                ...
                >
                  {currentStep.options.map((option) => (
                    <button
                      key={option.id}
                      onClick={() => handleOptionSelect(option.label, option.nextStepId)}
                      className="px-4 py-2 text-sm bg-white border border-lavender-200 text-lavender-700 rounded-full hover:bg-lavender-50 hover:border-lavender-300 transition-all"
                    >
                      {option.label}
                    </button>
                  ))}
            ...
            <div className="p-3 bg-gray-50 border-t border-gray-100 text-center">
              <p className="text-xs text-gray-500">
                Need help?{' '}
                <a href="#get-help" onClick={() => setIsOpen(false)} className="text-lavender-600 hover:underline">
                  Contact us directly
                </a>
              </p>
            </div>
